...
function standard_define() {
    ...
    if (nin > 0) {
        pin = new ScilabDouble(zeros(nin));
    }
    var nout = model.out.length;
    if (nout > 0) {
        pout = new ScilabDouble(zeros(nout));
    }
    var ncin = model.evtin.length;
    if (ncin > 0) {
        pein = new ScilabDouble(zeros(ncin));
    }
    var ncout = model.evtout.length;
    if (ncout > 0) {
        peout = new ScilabDouble(zeros(ncout));
    }
